    ...
    ui(new Ui::MainWindow)
{
	...
	ui->toolButton_add_call->setVisible(false);
...
{
     QDesktopServices::openUrl(QUrl("https://github.com/qwxingren/tjphone"));
}
...
void MainWindow::save_uri_history()
{
	...
	{
		snprintf(key,sizeof(key),"uri%i",i);
		lp_config_set_string(cfg,"QtUi",key,completer->currentCompletion().toLocal8Bit().data());
		if (i>10) break;
	}
	lp_config_sync(cfg);
}
...
{
	if (linphone_core_invite(linphone_qt_get_core(),uri.toLocal8Bit().data())!=NULL) {
...
void MainWindow::update_call_buttons(LinphoneCall *call)
{
	...
	ui->toolButton_call->setVisible(!add_call);
	...
	ui->toolButton_add_call->setVisible(add_call);
