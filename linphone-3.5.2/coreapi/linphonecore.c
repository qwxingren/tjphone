...
#define ROOT_CA_FILE PACKAGE_DATA_DIR "/Yt/rootca.pem"
...
static void sip_config_read(LinphoneCore *lc)
{
	...
	lc->sip_conf.auto_net_state_mon=lp_config_get_int(lc->config,"sip","auto_net_state_mon",0);
...
static void rtp_config_read(LinphoneCore *lc)
{
	...
	linphone_core_set_nortp_timeout(lc,30);
	...
	linphone_core_set_rtp_no_xmit_on_audio_mute(lc,FALSE);
}
    ...
    const char *factory_config_path, void * userdata)
{
	...
	memset(&lc->vtable,0,sizeof(LinphoneCoreVTable));
...
static void monitor_network_state(LinphoneCore *lc, time_t curtime){
	...
	SalAddress *dest_parsed=NULL;
	/* only do the network up checking every five seconds */
	if (last_check==0 || (curtime-last_check)>=5){
		if(lc->default_proxy){
			dest = linphone_proxy_config_get_addr(lc->default_proxy);
			dest_parsed=sal_address_new(dest);
			dest = sal_address_get_domain(dest_parsed);
		}
		linphone_core_get_local_ip_for(lc->sip_conf.ipv6_enabled ? AF_INET6 : AF_INET, dest, result);
	...
	sal_address_destroy(dest_parsed);
}
...
LinphoneCall * linphone_core_invite2(LinphoneCore *lc, const char *url){
	LinphoneCall *call;
	LinphoneCallParams *p=linphone_core_create_default_call_parameters (lc);
	p->has_video &= !!lc->video_policy.automatically_initiate;
	call=linphone_core_invite_with_params2(lc,url,p);
	linphone_call_params_destroy(p);
	return call;
}
...
LinphoneCall * linphone_core_invite_with_params2(LinphoneCore *lc, const char *url, const LinphoneCallParams *p){
	LinphoneAddress *addr=linphone_core_interpret_url(lc,url);
	if (addr){
		LinphoneCall *call;
		call=linphone_core_invite_address_with_params2(lc,addr,p);
		linphone_address_destroy(addr);
		return call;
	}
	return NULL;
...
LinphoneCall * linphone_core_invite_address2(LinphoneCore *lc, const LinphoneAddress *addr){
	LinphoneCall *call;
	LinphoneCallParams *p=linphone_core_create_default_call_parameters(lc);
	p->has_video &= !!lc->video_policy.automatically_initiate;
	call=linphone_core_invite_address_with_params2 (lc,addr,p);
 ...
 * @return a LinphoneCall object or NULL in case of failure
**/
LinphoneCall * linphone_core_invite_address_with_params2(LinphoneCore *lc, const LinphoneAddress *addr, const LinphoneCallParams *params)
{
	const char *route=NULL;
	...
	call=linphone_call_new_outgoing(lc,linphone_address_clone(addr),parsed_url2,params);
...
int linphone_core_resume_call(LinphoneCore *lc, LinphoneCall *the_call)
{
	...
	snprintf(temp,sizeof(temp)-1,"Resuming the call with %s",linphone_call_get_remote_username_as_string(call));
	if (lc->vtable.display_status)
