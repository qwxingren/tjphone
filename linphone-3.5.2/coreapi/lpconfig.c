...
LpConfig *lp_config_parse_str(char *str)
{
    char            tmp[MAX_LEN];
    LpSection      *cur = NULL;
    char           *pos1,
    ...
    char           *line;
    int             pseek = 0;
	LpConfig *lpconfig=NULL;
	if (str == NULL) return;
    /*
     * found section 
     */
	lpconfig=lp_new0(LpConfig,1);
	if(lpconfig==NULL) return NULL;
    cur = lp_config_find_section(lpconfig, "default");
    if (cur == NULL) {
		cur = lp_section_new("default");
		lp_config_add_section(lpconfig, cur);
    } else {
		ms_warning("parse error!");
    }
    ...
    while (sscanf(line, "%1023[^\n\r]", tmp)) 
	{
		pseek = pseek + strlen(tmp) + 1;
		line = str + pseek;
		pos1 = strchr(tmp, ':');
		if (pos1 != NULL) 
		{
